...
let make_symbol ~name ~kind ~node_idx ~(ty : Ast.typ option)
    ~(loc : Location.location option) symtab =
  { name; kind; node_idx; ty; loc; declared_in = current_scope_kind symtab }

let add_var (symtab : t) ~name ~node_idx ~(ty : Ast.typ option)
    ~(loc : Location.location option) : t =
...
let rec build_expr (symtab : t) (expr : expr) : t =
  ...
  | NewExpr x ->
      List.fold_left (fun acc f -> build_expr acc f.value) symtab x.fields
  | CreateExpr x ->
      x.params |> List.fold_left build_expr (build_expr symtab x.coroutine)
...
let rec build_stmt (symtab : t) (stmt : stmt) : t =
  ...
  | LetStmt x ->
      ...
      add_var symtab ~name:x.var_name ~node_idx:x.node_idx ~ty:(Some x.var_type)
        ~loc:(Some x.loc)
  | ForLoop x ->
      let symtab = enter_scope symtab BlockScope in
      let symtab =
        add_var symtab ~name:x.iter_var ~node_idx:x.node_idx ~ty:None
          ~loc:(Some x.loc)
      in
  ...
  | IfResumeStmt x ->
      ...
      let symtab_entered_if_block =
        enter_scope symtab_after_coroutine_expr BlockScope
      in

      let symtab_with_var_in_block =
        Option.fold ~none:symtab_entered_if_block
          ~some:(fun var_name_str ->
            add_var symtab_entered_if_block ~name:var_name_str
              ~node_idx:x.node_idx ~ty:None ~loc:(Some x.loc))
          x.var
      in
  ...
  | AliasStmt x ->
      let alias_info =
        make_symbol ~name:x.type_name ~kind:Struct ~node_idx:x.node_idx
          ~ty:(Some x.other_type) ~loc:(Some x.loc) symtab
      in
...
and build_fn_decl (symtab : t) (fn : fn_decl) : t =
  let fn_info =
    make_symbol ~name:fn.name ~kind:Function ~node_idx:fn.node_idx
      ~ty:
        (Some
           (FnType
              {
                args = List.map (fun arg -> arg.arg_type) fn.args;
                ret = fn.ret_type;
                node_idx = fn.node_idx;
                loc = fn.loc;
              }))
        (* Corrected to arg.arg_type *)
      ~loc:(Some fn.loc) symtab
  in
  ...
  let symtab =
    List.fold_left
      (fun (acc : t) (p : Ast.argument) ->
        add_var acc ~name:p.name ~node_idx:p.node_idx ~ty:(Some p.arg_type)
          ~loc:(Some p.loc)) (* Corrected to p.arg_type *)
      symtab fn.args
  in
...
and build_co_decl (symtab : t) (co : co_decl) : t =
  let co_info =
    make_symbol ~name:co.name ~kind:Function ~node_idx:co.node_idx
      ~ty:
        (Some
           (CoType
              {
                args = List.map (fun arg -> arg.arg_type) co.args;
                yield = co.yield_type;
                node_idx = co.node_idx;
                loc = co.loc;
              }))
        (* Corrected to arg.arg_type *)
      ~loc:(Some co.loc) symtab
  in
...
and build_struct_decl (symtab : t) (s : struct_decl) : t =
  let struct_info =
    make_symbol ~name:s.name ~kind:Struct ~node_idx:s.node_idx ~ty:None
      ~loc:(Some s.loc) symtab
  in

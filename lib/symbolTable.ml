...
let exit_scope symtab =
  ...
  | Some parent ->
      ...
      symtab  (* <-- return the updated symbol table *)
  | None ->
      raise (Invalid_argument "symbolTable: Cannot exit global scope")
...
let rec fold_scope scope ~init ~f =
  ...
  List.fold_left (fun acc child -> fold_scope child ~init:acc ~f) acc !(scope.children)

let fold_symbols t ~init ~f =
  fold_scope t.root ~init ~f
...
let rec collect_scopes_by_kind scope kind_filter acc =
  ...
  List.fold_left (fun acc child -> collect_scopes_by_kind child kind_filter acc) acc' !(scope.children)

let scopes_by_kind t kind_filter =
  collect_scopes_by_kind t.root kind_filter []
...
let rec dump_scope buf indent scope =
  ...
  let scope_header = match scope.kind with
  ...
  Hashtbl.iter (fun name (info : symbol_info) ->
    let kind_str = string_of_symbol_kind info.kind in
    Buffer.add_string buf (indent_str ^ "  " ^ name ^ " [" ^ kind_str ^ "] node_idx=" ^
...
let rec find_struct_scope scope name =
  ...
  | _ ->
    List.find_map (fun child -> find_struct_scope child name) !(scope.children)

let rec find_child_scope_by_kind (parent_scope : scope) (kind_filter : scope_kind) : scope option =
  List.find_map (fun child ->
    if child.kind = kind_filter then Some child
    else find_child_scope_by_kind child kind_filter (* Recursively search in children of children *)

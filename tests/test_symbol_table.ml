...
let typ : Ast.typ Alcotest.testable =
...
let test_basic_let () =
  ...
  | None ->
      failwith "Variable 'x' not found in root scope"
...
let test_function_declaration () =
  ...
  | None ->
      failwith "Function 'add' not found in root scope"
(* 
...
let test_nested_scopes () =
  ...
  let symtab = build_symbol_table program in
  let root_scope_obj = root_scope symtab in (* Use the new root_scope API *)

  (* Check global variable *)
  (match find_in_scope root_scope_obj "x" with (* Use the new find_in_scope API *)
   | Some sym ->
  ...
  let fn_sym =
    ...
    | Some sym ->
        ...
        sym
    | None ->
        failwith "Function 'test_fn' symbol not found in root scope"
  in

  (* Find the actual function scope object using `scopes_by_kind` *)
  let fn_scope_obj =
    match scopes_by_kind symtab (FunctionScope "test_fn") with
    | [s] -> (* Expecting exactly one function scope named "test_fn" *)
        s
    | [] -> failwith "Function scope 'test_fn' not found in symbol table"
    | _ -> failwith "Multiple function scopes named 'test_fn' found, expected one"
  in
  ...
  (match find_in_scope fn_scope_obj "x" with
   | Some sym ->
       ...
       check scope_kind "Local var declared in" (FunctionScope "test_fn") sym.declared_in;
       print_endline "✓ Local shadowed variable 'x' found in function"
   | None ->
       failwith "Local variable 'x' not found in function");
